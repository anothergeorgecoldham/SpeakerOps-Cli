...
- `speakerops.config` manages `.speakerops\speakerops.yaml`, model settings, security settings, and environment overrides. `speakerops init` writes the packaged profile template and creates `talks\`.
- `speakerops.config` also discovers the project root from nested directories and stores the current talk path in `.speakerops\speakerops.yaml`.
...
- `speakerops.security` enforces local hardening controls: operator authorization, generated-content secret scanning, and provenance records for generated artefacts.
...
- This repository is now an early safety-focused MVP. Preserve workspace boundaries, path denylisting, network allowlisting, approval gates, audit logging, tool allowlisting, secret scanning, trust wrapping, and provenance unless a task explicitly changes the security model.
...
- `speakerops new` sets the created talk as the current talk. `chat`, `research`, `generate cfp`, `generate outline`, `review`, and `trust` default to the current talk when no talk path is provided.
